...
1) Extracting the features from a RGB geotif file and a chm shapefile using the label geodatabase. This work is done in the features notebook, auxiliary functions are stored in data_extraction.py and preprocessing_aux.py. The extracted features are first stored in a pandas dataframe, then saved in a csv file.
File for Part I:
    - features.ipynb
    - data_extraction.py
